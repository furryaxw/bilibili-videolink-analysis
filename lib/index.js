...
function apply(ctx, config) {
    ...
    function isLinkProcessedRecently(ret, lastProcessedUrls, config, logger, session) {
        ...
        if (lastProcessedUrls[lastretUrl] && (currentTime - lastProcessedUrls[lastretUrl] < config.MinimumTimeInterval * 1000) && lastProcessedUrlSession[lastretUrl] == event_id) {
            if (config.loggerinfo) {
        ...
        lastProcessedUrlSession[lastretUrl] = event_id;
        return false; // 没有处理过
    }
