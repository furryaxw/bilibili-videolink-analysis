...
function apply(ctx, config) {
    ...
    ctx.middleware(async (session, next) => {
        ...
        if (links) {
            ...
            if (ret && !isLinkProcessedRecently(ret, lastProcessedUrls, config, logger, session)) {
    ...
    //判断链接是否已经处理过
    function isLinkProcessedRecently(ret, lastProcessedUrls, config, logger, session) {
        const lastretUrl = extractLastUrl(ret); // 提取 ret 最后一个 http 链接作为解析目标
        const currentTime = Date.now();
        const event_id = session.event.channel.id
        if (config.loggerinfo) {
            logger.info(`Session: ${event_id}`)
            logger.info(lastProcessedUrls[event_id])
        }
        if (!lastProcessedUrls[event_id]){
            lastProcessedUrls[event_id] = {}
        }
        if (currentTime - lastProcessedUrls[event_id][lastretUrl] < config.MinimumTimeInterval * 1000) {
            if (config.loggerinfo) {
        ...
        // 更新该链接的最后处理时间
        lastProcessedUrls[event_id][lastretUrl] = currentTime;
        return false; // 没有处理过
    }
